...
export function claimDto(model: Repository, claim: ClaimRecord): ClaimDto {
...
// The generic browse-list for a single entity kind (components, flows, runbooks, decisions, ...). Same
// card shape as featureList; the portal's list tabs render these. An honest empty list for a kind the
// repo has none of — never a placeholder.
export function entityList(model: Repository, repoId: string, kind: EntityKind): EntityListDto {
  return { kind, entities: model.listEntities(repoId, kind).map((e) => entityCard(model, e)) };
}
...
// Optimistic-concurrency tag for a review item: a stable hash over ONLY its mutable decision state.
// The client echoes it as `expected_version` on a mutation; the server recomputes it and rejects a
// mismatch with 409. Kept here (not in review.ts) so both the read model and the mutation handler
// derive the version identically — a single source of truth for "has this item changed?".
export function reviewItemVersion(item: ReviewItemRecord): string {
  const material = [
    item.status,
    item.claim_id,
    item.assigned_to ?? "",
...
export function reviewItemDto(model: Repository, item: ReviewItemRecord): ReviewItemDto {
  const claim = model.getClaim(item.claim_id);
  const entity = claim ? model.getEntity(claim.entity_id) : null;
  // The claim currently occupying this slot, if it is a DIFFERENT live claim (a contradiction the
  // proposed claim would displace). If the slot's current claim is the proposed one itself, there is
  // no distinct "current knowledge" to show — an honest null, never the proposal echoed back.
  const slot = claim ? model.currentClaimInSlot(claim.entity_id, claim.claim_kind) : null;
  const current = slot && slot.claim_id !== item.claim_id ? slot : null;
  const evidence = claim
    ? model.evidenceForClaim(claim.claim_id).map(({ evidence: e, stance }) => ({
        evidence_id: e.evidence_id,
        source_type: e.source_type,
        source_uri: e.source_uri,
        path: e.path,
  ...
  return {
    ...
    claim_review_policy: claim?.review_policy ?? "automatic",
    current_claim_id: current?.claim_id ?? null,
    current_claim_content: current?.normalized_content ?? null,
    evidence,
    reason: item.reason,
    ...
    version: reviewItemVersion(item),
  };
}

export function reviewItems(model: Repository, repoId: string, status?: ReviewItemDto["status"]): ReviewItemDto[] {
  return model.reviewItemsForRepository(repoId, status).map((item) => reviewItemDto(model, item));
}
...
export function buildOverview(
  ...
  // The team panel is supplied by the caller when a workspace actually answered. It is a PARAMETER, not
  // a fetch: this builder stays pure and the local overview is computed with no dependency on the
  // workspace being reachable, which is what keeps local context working during a workspace outage.
  team: TeamMetricsPanelDto | null = null,
): OverviewDto {
  ...
  return {
    ...
    integrations: [],
    attention,
    // A local install has no workspace, and null says exactly that. A caller passes a panel only when a
    // workspace actually answered — a workspace OUTAGE also leaves it null, which is why the local
    // overview above is computed entirely without it.
    team,
  };
}

// ---------------------------------------------------------------------------------------------
// team (workspace) metrics projection
// ---------------------------------------------------------------------------------------------

/**
 * Project a team metrics report onto portal metric cards. The projection ADDS no arithmetic: every value
 * is carried through exactly as the workspace measured (or withheld) it, and the exactness label is
 * derived from the report itself —
 *   - `exact`      : a dollar figure computed from receipts measured on both sides;
 *   - `cohort`     : a behaviour trend over a cohort at or above the privacy floor;
 *   - `structural` : a coverage/count ratio that needs no measurement to be true;
 *   - `unavailable`: nothing measured it, OR it is withheld — and `suppression_reason` distinguishes
 *                    those two cases so "we did not measure" never reads as "we will not tell you".
 */
export function teamMetricsPanel(report: TeamMetricsReport): TeamMetricsPanelDto {
  const suppression = report.suppression_reason;
  /** A withheld trend carries the reason; an unmeasured one carries null. */
  const withheld = (value: number | null): string | null =>
    value === null && suppression !== null ? suppression : null;

  const exactCost = report.exact_cost.receipts > 0
    ? report.exact_cost.total_net_input_cost_delta_usd
    : null;
  const timeTrend = report.time_to_verified_change?.p50_ms ?? null;
  ...
  const metrics: MetricDto[] = [
    ...
    {
      id: "team_measurement_coverage",
      label: "Measurement coverage",
      value: report.measurement_quality.coverage,
      unit: "percent",
    ...
    {
      id: "team_latency",
      label: "Context latency (p95)",
      value: report.latency.p95_ms,
      unit: "milliseconds",
    ...
    {
      id: "team_verified_reuse",
      label: "Verified reuse",
      value: report.verified_reuse.rate,
      unit: "percent",
    ...
    {
      id: "team_review_burden",
      label: "Review burden per task",
      value: report.review_burden.decisions_per_task,
      unit: "count",
    ...
    {
      id: "team_failed_open",
      label: "Failed-open rate",
      value: report.failed_open.rate,
      unit: "percent",
