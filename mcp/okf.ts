...
// The OKF BUNDLE tooling that used to live below this line — okfBundleDir, migratePacketsToOkf,
// loadOkfConcepts, lintOkfConcept, lintOkfBundle, okfConceptFileName and a self-contained HTML
// viewer — has been removed, along with the `kage okf` command tree that was its only caller.
//
// It existed to export the packet store as a browsable OKF bundle and read it back. In two months
// no third-party OKF consumer ever materialised to read one, and the "lossless" round trip was
// paid for by a fenced kage-state JSON block that doubled every packet and once leaked raw JSON
// into a card claim. DIRECTION.md put it on the kill list; this is that decision applied.
//
// What remains above is the part that is NOT tooling: packetToOkfConcept / okfConceptToPacket are
// how the packet store reads and writes its own .md files, so they are the format itself.
