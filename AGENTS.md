...
- [decision] Compiler uses a proxy-loopback AI provider so it never needs to hold an API key — The compiler's semantic pass runs AI calls through a proxy-loopback provider rather than holding a provider API key directly. (card_0e36e9b0)
- [decision] Desktop app deleted its own backend server, talks to a worker instead — The desktop app's local backend server was removed entirely; the app now communicates with a worker process rather than a server. (card_00af195e)
- [caution] kage-sync used to delete concurrently-added memory packets — kage-sync had a race condition where, if a memory packet was added concurrently with a sync run, the sync process would delete it rather than merging it in. (card_836761de)
- [caution] Packaged desktop app silently lost every derived route — When the desktop app was packaged (as opposed to run from source), it silently lost all of its derived routes — a packaging-only failure that a source checkout's tests could not catch. (card_b1e8129a)
- [caution] A computed metric was written but never drained, so it read as permanently absent — A measurement pipeline computed and wrote its value correctly, but nothing ever drained/read that write path, so the UI displayed the metric as 'absent' even though the data existed. (card_2ffa47a5)
- [caution] /v2/work took 38 seconds and the app sat on it, blocking the UI — The Board's /v2/work endpoint had a request latency of 38 seconds, and the app synchronously waited on it, freezing the UI during that window. (card_fa02a676)
<!-- kage:end -->
