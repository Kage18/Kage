...
<!-- kage:begin -->
<!-- 6 cards; generated by Kage — edit cards, not this block -->
- [caution] A packaged feature can pass every source-checkout test yet be 404-broken from npm — verify from an installed tarball — 4.0.0 shipped the knowledge portal (kage viewer -> /app/) broken for every npm user, while working perfectly from a source checkout, so no test caught it. (card_0062b296)
- [caution] ASSIST mode live-verified on subscription OAuth: mutated bodies are accepted, first delivered deliveries recorded — SETTLES the last open live question from the OAuth proxy saga: ASSIST mode — where the proxy MUTATES the request body by appending verified repo memory to the last user turn — is accepted by Anthropic on a Claude Code SUBSCRIPTION (OAuth, not API key). (card_03113dbe)
- [caution] Claude Code plugin schema: no displayName; hooks/commands ship via plugin dir — claude plugin validate rejects unrecognized keys in plugin/.claude-plugin/plugin.json — 'displayName' fails validation (use 'name' only). (card_916afcb3)
- [caution] gitPathToProjectRelative spawns git per call — never use it in per-line git log loops — gitPathToProjectRelative (mcp/kernel.ts) calls gitProjectPrefix, which runs 'git rev-parse --show-prefix' via execFileSync on every invocation, plus an existsSync fallback. (card_d7ccb605)
- [caution] ROOT CAUSE + FIX: kage proxy 429 on subscription was the OAuth system-prompt guardrail; inject into the user message instead — Diagnosed the consistent 429 rate_limit_error on POST /v1/messages?beta=true (6 of 6 requests, every Claude Code retry) seen when running the proxy on subscription auth. (card_92e9f62b)
- [caution] Validate ISO calendar fields before Date.parse — Protocol boundary validators must not rely on Date.parse alone for ISO timestamp validity because JavaScript normalizes impossible calendar dates such as 2026-02-30 into March. (card_2afdf9b4)
<!-- kage:end -->
