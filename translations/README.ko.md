...
### 사라지지 않는 코딩 에이전트 팀 메모리

<img src="../docs/kage-viewer.jpg" alt="kage viewer: 팀이 포착한 결정·런북·버그 수정이 관련 코드에 연결됨" width="760">

<sub>`kage viewer`: 당신 팀의 결정·런북·버그 수정(보라)이 저장소에 보관되고 관련 코드(파랑)에 연결됩니다.</sub>

코드 뒤에 있는 결정, 까다로운 배포의 런북, 고약한 버그의 근본 원인. 이런 지식은 사람들의
머릿속에만 있고 채팅 속으로 흘러가 버린 뒤 사라집니다. **Kage** 는 당신의 코딩 에이전트가
일하는 동안 그것을 포착해 저장소 안에 일반 파일로 보관하고, git 을 통해 팀 전체와 공유합니다.
다음 세션(당신 또는 동료의)은 이미 그것을 아는 상태로 시작합니다. 모든 메모리는 실제 코드와도
대조해 검증되므로, 공유되는 내용은 계속 사실로 남습니다. 계정 불필요, 데이터베이스 불필요,
API 키 불필요.
...
- **협업적입니다.** 한 사람(또는 그의 에이전트)이 알아낸 지식이 팀 전체의 것이 됩니다.
  메모리는 git 으로 공유되므로, 동료의 다음 세션은 당신이 방금 배운 것에서 시작합니다.
  백지에서 시작하지 않습니다.
- **git 네이티브입니다.** 메모리는 저장소 안의 일반 파일로, 코드와 같은 PR 에서 리뷰되며,
  한 대의 머신이나 벤더의 클라우드에 갇히지 않습니다. 당신의 지식은 계속 당신의 것입니다.
