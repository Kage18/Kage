...
### 失われない、コーディングエージェントのためのチームメモリ

<img src="../docs/kage-viewer.jpg" alt="kage viewer：チームが捕捉した意思決定・ランブック・バグ修正が、関係するコードに結ばれている" width="760">

<sub>`kage viewer`：チームの意思決定・ランブック・バグ修正（紫）がリポジトリに保存され、関係するコード（青）に結ばれています。</sub>

コードの背後にある意思決定、厄介なデプロイのランブック、嫌なバグの根本原因。こうした知識は
人の頭の中にしかなく、チャットを流れて消え、やがて失われます。**Kage** はそれを、あなたの
コーディングエージェントが作業する中で捕捉し、リポジトリ内にプレーンなファイルとして保存し、
git を通じてチーム全員と共有します。次のセッション（あなた自身、あるいはチームメイトの）は、
すでにそれを知った状態で始まります。各メモリは実際のコードとも照合されるため、共有される内容は
真であり続けます。アカウント不要、データベース不要、API キー不要。
...
他のメモリツールと異なる点が 3 つあります：

- **協働的。** 一人（またはそのエージェント）が解明した知識が、チーム全員のものになります。
  メモリは git を通じて共有されるので、チームメイトの次のセッションは、あなたが今学んだ
  ことから始まります。白紙からではありません。
- **git ネイティブ。** メモリはリポジトリ内のプレーンなファイルで、コードと同じ PR で
  レビューされ、1 台のマシンやベンダーのクラウドに閉じ込められることはありません。
  あなたの知識は、あなたのものであり続けます。
...
- **LongMemEval-S 検索：** 98.72% R@10 / 99.79% R@20、依存ゼロ。
