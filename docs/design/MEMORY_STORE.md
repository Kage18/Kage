...
**Landed 2026-08-20, M4.** Kage is tuned for repos under roughly 10,000 indexable files
— not, measured, the point where cold index or warm refresh gets painful (`docs/BENCHMARKS.md`'s
M4 section: both stay in the single-to-tens-of-seconds range even at 10,000 files on this
machine), but the largest size these benchmarks actually ran, and the size at which recall
latency — paid on every single `kage_context` call, not once per refresh — has already
drifted roughly 8-13x past a feels-instant budget. `kage scan` (`mcp/cli.ts:557`) and
`kage install` (`mcp/cli.ts:781`) are the two places a person first learns what Kage thinks
of their repo; `scan` reuses the file count `truthReport` already computed
(`result.totals.files_scanned`, `mcp/cli.ts:594`), and `install` calls the new cheap,
parse-free `countIndexableFiles()` once (`mcp/cli.ts:829`) — both pass that count through
`scaleGuardMessage()` (`mcp/kernel.ts`) and print its result, once, in their default
human-readable output only, when the count exceeds `SCALE_GUARD_FILE_THRESHOLD`. Not a
block, not a silent degradation — a stated envelope, the same discipline
`MemoryOverview.measured`-style absence-is-a-fact reporting already applies elsewhere in
this codebase (`CONTEXT_ENGINE.md`'s own citation of that pattern applies here too).
...
| **M4 — benchmarks and the scale guard** | **Landed 2026-08-20.** `mcp/bench/fixture.ts` generates a deterministic, seeded synthetic repo fixture (N TypeScript files with a real acyclic import/call graph, N⁄10 memory packets citing them); `mcp/bench/harness.ts`'s `runBenchmark()` drives cold `indexProject()`, warm `refreshProject()` (after touching 1% of files), one `recall()`, and one `queryStructuralGraphFromStore()` call against it, on a forced backend; `mcp/bench/run-one.ts`/`mcp/bench/run.ts` are the CLI entry points that ran it for real at 1,000 and 10,000 files on both backends (`docs/BENCHMARKS.md`'s new "Memory store scale" section) — neither backend hit the 900-second timeout, so there is no DNF to report at this scale. `mcp/kernel.ts` exports the guard itself: `countIndexableFiles()` (the same file walk `buildStructuralIndex` uses, without parsing) and `scaleGuardMessage()`, compared against `SCALE_GUARD_FILE_THRESHOLD = 10000` — not "the point cold index gets painful" (measured, it isn't, at this size) but the largest size these benchmarks actually ran, chosen because recall latency (not cold index or warm refresh) is the first of the four measured numbers to drift meaningfully at exactly this size. `mcp/cli.ts`'s `scan` and `install` commands print `scaleGuardMessage()`'s result once, in their default human-readable output only, when a repo's file count exceeds the threshold — never blocking, never repeated (no other command calls it). | **Met.** `mcp/scale-guard.test.ts` covers: fixture determinism (same seed produces byte-identical generated files, via a sha256 fingerprint over every generated file); the harness running end-to-end on both backends at 200 files (fast enough for `npm test`; the 10,000-file run that produced `docs/BENCHMARKS.md`'s numbers is not part of the suite, run once by hand instead); `scaleGuardMessage()` firing above its threshold and not at-or-below it, on both the default and an overridden threshold; the guard's message naming `docs/BENCHMARKS.md`; and every backtick-quoted path `docs/BENCHMARKS.md`'s own M4 section cites actually existing in the worktree. `docs/BENCHMARKS.md` states real before/after numbers from the fixture (not projections) and marks each of this row's original targets (cold index in minutes, warm refresh in seconds, recall under 100ms, peak heap under 500MB) met or not-yet-met individually, with the measured reason why, rather than silently omitting the ones that missed. |
